...
export default function ServicePageClient({ service, market = 'GLOBAL', alternateUrl }: ServicePageClientProps) {
    ...
    const customSections = service.customSections ?? [];
    const theme = resolveServiceTheme(service.theme, customSections.map((c) => c.id));
    const themeStyle = buildThemeStyle(service.theme);
    ...
    const heroTitle = (
        <>
            {titleWords.map((word, i) => (
                <span key={`${word}-${i}`} className={i >= titleWords.length - 2 ? 'text-accent' : ''}>
                    {word}{' '}
                </span>
            ))}
        </>
    ...
    const sec = (key: string) => {
        const cfg = theme.sections.find((s) => s.key === key);
        return `${cfg ? SECTION_BG_CLASS[cfg.background ?? 'default'] : ''} ${densityClass}`.trim();
    };

    /** Render an admin-authored custom section in one of the on-brand layouts. */
    const renderCustomSection = (cs: CustomSection): React.ReactNode => {
        const heading = cs.heading?.trim();
        const paragraphs = (cs.body ?? '').split(/\n{2,}/).map((p) => p.trim()).filter(Boolean);
        const items = cs.items ?? [];

        if (cs.layout === 'cta') {
            return (
                <SectionWrapper className={`text-center ${densityClass}`.trim()}>
                    <div className="max-w-3xl mx-auto py-14 px-8 rounded-3xl bg-accent-gradient shadow-glow relative overflow-hidden">
                        ...
                        <div className="relative z-10">
                            {heading && <h2 className="text-3xl md:text-4xl font-bold text-white mb-6">{heading}</h2>}
                            {paragraphs.map((p, i) => <p key={i} className="text-white/90 text-lg mb-4 max-w-xl mx-auto">{p}</p>)}
                            <Button onClick={() => openModal(`Custom CTA - ${heading ?? cs.id}`)} variant="primary" size="lg" className="mt-4 !bg-neutral-950 !text-white !bg-none hover:!bg-black border border-white/10 hover:border-white/20 shadow-2xl font-bold">
                                {cs.buttonText?.trim() || 'Get in touch'}
                            </Button>
                        </div>
                    </div>
                </SectionWrapper>
        ...
        return (
            <SectionWrapper className={sec(cs.id)}>
                <div className={cs.layout === 'cards' ? '' : 'max-w-4xl mx-auto'}>
                    {(cs.eyebrow || heading || cs.subheading) && (
                        <div className="text-center mb-12">
                            {cs.eyebrow && <SectionBadge label={cs.eyebrow} />}
                            {heading && <h2 className="text-3xl md:text-4xl font-bold text-foreground mb-4">{heading}</h2>}
                            {cs.subheading && <p className="text-foreground-secondary text-lg max-w-2xl mx-auto">{cs.subheading}</p>}
                        </div>
                    )}

                    {cs.layout === 'text' && (
                        <div className="max-w-3xl mx-auto text-center space-y-4">
                            {paragraphs.map((p, i) => <p key={i} className="text-foreground-secondary text-lg leading-relaxed">{p}</p>)}
                        </div>
                    )}

                    {cs.layout === 'cards' && items.length > 0 && (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 max-w-6xl mx-auto">
                            {items.map((it, i) => (
                                <div key={i} className="rounded-2xl border border-border bg-background-card p-6 hover:border-accent/40 hover:shadow-card-hover transition-all duration-300 group">
                                    <h3 className="text-lg font-bold text-foreground mb-2 group-hover:text-accent transition-colors">{it.title}</h3>
                                    {it.description && <p className="text-foreground-secondary text-sm leading-relaxed">{it.description}</p>}
                                </div>
                            ))}
                        </div>
                    )}

                    {cs.layout === 'list' && items.length > 0 && (
                            ...
                            {items.map((it, i) => (
                                <div key={i} className="flex items-start gap-4 p-5 rounded-2xl border border-border bg-background-card hover:border-accent/40 hover:shadow-card transition-all duration-300 group">
                                    <span className="mt-1 flex-shrink-0 w-7 h-7 rounded-full bg-accent/10 flex items-center justify-center group-hover:bg-accent/20 transition-colors">
                                        <svg width="14" height="14" viewBox="0 0 24 24" fill="none" className="text-accent"><path d="M20 6L9 17l-5-5" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" /></svg>
                                    </span>
                                    <span>
                                        <span className="block text-foreground font-semibold leading-snug">{it.title}</span>
                                        {it.description && <span className="block text-foreground-secondary text-sm mt-1 leading-relaxed">{it.description}</span>}
                                    </span>
                                </div>
                            ))}
                        </div>
    ...
    // ----- body sections (rendered in admin-configured order) -------------
    const renderSection = (key: string): React.ReactNode => {
        switch (key) {
            case 'metrics':
                return service.metrics && service.metrics.length > 0 ? (
                    <SectionWrapper className={`!py-0 mb-8 ${SECTION_BG_CLASS[theme.sections.find((s) => s.key === 'metrics')?.background ?? 'default']}`.trim()}>
            ...
            case 'problem':
                return service.challenges.length > 0 ? (
                    <SectionWrapper className={sec('problem')}>
                        <div className="max-w-4xl mx-auto">
                            <div className="text-center mb-12">
                                <SectionBadge label="The Problem" />
                                <h2 className="text-3xl md:text-4xl font-bold text-foreground mb-4">{service.problemHeading ?? 'Challenges You Face'}</h2>
                            </div>
                                ...
                                {service.challenges.map((challenge, i) => (
                                    <div key={i} className="flex items-start gap-4 p-5 rounded-2xl border border-border bg-background-card hover:border-red-500/30 hover:shadow-card transition-all duration-300 group">
                                        <span className="mt-1 flex-shrink-0 w-7 h-7 rounded-full bg-red-500/10 flex items-center justify-center group-hover:bg-red-500/20 transition-colors">
                                            <svg width="14" height="14" viewBox="0 0 24 24" fill="none" className="text-red-500/70"><path d="M18 6L6 18M6 6l12 12" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round" /></svg>
                                        </span>
            ...
            case 'solutions':
                return service.solutions.length > 0 ? (
                    <SectionWrapper className={sec('solutions')}>
                        <div className="max-w-4xl mx-auto">
                            <div className="text-center mb-12">
            ...
            case 'subServices':
                return service.subServices ? (
                    <SectionWrapper className={sec('subServices')}>
                        <div className="text-center mb-12">
            ...
            case 'process':
                return service.process ? (
                    <SectionWrapper className={sec('process')}>
                        <div className="max-w-4xl mx-auto">
                            <div className="text-center mb-16">
                                <SectionBadge label="Our Framework" />
                                <h2 className="text-3xl md:text-4xl font-bold text-foreground mb-4">{service.processHeading ?? 'Our Methodology'}</h2>
                            ...
                            <div className="space-y-0">
                                {service.process.map((step, i) => (
                                    <div key={i} className="flex flex-col md:flex-row gap-6 md:gap-10 items-start relative group py-8 first:pt-0 last:pb-0">
                                        ...
                                        <div className="flex-shrink-0 w-16 h-16 md:w-20 md:h-20 rounded-2xl bg-background border-2 border-border flex items-center justify-center text-accent text-2xl font-black shadow-sm group-hover:border-accent group-hover:shadow-[0_0_20px_rgba(var(--accent-rgb),0.15)] transition-all duration-300 z-10 relative">
                                            0{i + 1}
                                        </div>
            ...
            case 'pricing':
                return service.pricing ? (
                    <SectionWrapper className={sec('pricing')}>
                        <div className="max-w-6xl mx-auto">
                            ...
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6 items-stretch">
                                {service.pricing.tiers.map((tier, i) => (
                                    <div key={i} className={`relative rounded-3xl p-8 flex flex-col transition-all duration-300 overflow-hidden ${tier.highlighted ? 'border-2 border-accent bg-background-card shadow-[0_12px_50px_rgba(var(--accent-rgb),0.15)] md:scale-[1.03] z-10' : 'border border-border bg-background-card shadow-card hover:border-accent/30 hover:shadow-card-hover'}`}>
                                        {tier.highlighted && (
                                            <>
                                                <div className="absolute top-0 left-0 right-0 h-1 bg-accent-gradient" />
                                                <span className="self-start mb-4 px-3 py-1 rounded-full bg-accent text-white text-[10px] font-black uppercase tracking-[0.15em]">Most Popular</span>
                                            </>
                                        )}
                                        <h3 className="text-xl font-bold text-foreground mb-1">{tier.name}</h3>
                                        {tier.tagline && <p className="text-foreground-secondary text-sm mb-6">{tier.tagline}</p>}
                                        <div className="mb-7">
                                            {tier.priceLabel && <p className="text-foreground-muted text-[10px] uppercase tracking-[0.2em] mb-2">{tier.priceLabel}</p>}
                                        ...
                                        <Button onClick={() => openModal(`Pricing - ${tier.name}`)} variant={tier.highlighted ? 'primary' : 'secondary'} size="md" className={`w-full justify-center ${tier.highlighted ? primaryBtnClass : ''}`}>
                                            {tier.buttonText}
                                        </Button>
                                    </div>
                                ))}
            ...
            case 'whyUs':
                return service.whyUs ? (
                    <SectionWrapper className={sec('whyUs')}>
            ...
            case 'techStack':
                return service.techStack.length > 0 ? (
            ...
            case 'comparison':
                return service.comparison ? (
                    <SectionWrapper className={sec('comparison')}>
                        <div className="max-w-4xl mx-auto">
                            <div className="text-center mb-12">
                            ...
                            <div className="rounded-2xl border border-border overflow-hidden shadow-card">
                                <div className="grid grid-cols-2">
                                    <div className="p-5 md:p-6 border-r border-border bg-red-500/5"><span className="font-bold text-foreground-secondary text-xs uppercase tracking-[0.15em]">Typical Agency</span></div>
                                    <div className="p-5 md:p-6 bg-accent/5"><span className="font-bold text-accent text-xs uppercase tracking-[0.15em]">Our Approach</span></div>
                                </div>
            ...
            case 'faqs':
                return faqs.length > 0 ? (
                    <SectionWrapper className={sec('faqs')}>
                        <div className="max-w-4xl mx-auto">
                            <div className="text-center mb-12">
            ...
            default: {
                ...
                return custom ? renderCustomSection(custom) : null;
            }
        }
    };
    ...
    return (
        <div className="pt-24 pb-16" style={themeStyle}>
            ...
            {theme.heroLayout === 'split' ? (
                <section className="relative px-6 md:px-8 pt-16 pb-8 overflow-hidden">
                    <div className="absolute inset-0 pointer-events-none" aria-hidden="true">
                        <div className="absolute top-[10%] right-0 w-[700px] h-[600px] bg-hero-glow animate-glow-pulse" />
                    </div>
                    <div className="relative z-10 max-w-container mx-auto grid lg:grid-cols-2 gap-12 items-center">
                        <div>
                            ...
                            <div className="flex flex-col sm:flex-row items-start gap-4 mt-10 animate-fade-in-up" style={{ animationDelay: '0.3s' }}>
                                <Button onClick={() => openModal('Hero')} variant="primary" size="lg" className={primaryBtnClass}>
                                    ...
                                    <svg width="16" height="16" viewBox="0 0 16 16" fill="none" aria-hidden="true" className="ml-2"><path d="M3 8H13M13 8L9 4M13 8L9 12" stroke="currentColor" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round" /></svg>
                                </Button>
                            </div>
                        </div>
                        {service.metrics && service.metrics.length > 0 && (
                            <div className="grid grid-cols-1 sm:grid-cols-3 lg:grid-cols-1 gap-4 animate-fade-in-up" style={{ animationDelay: '0.2s' }}>
                                {service.metrics.slice(0, 3).map((metric, i) => (
                                    <div key={i} className="rounded-2xl border border-border bg-background-card p-6 hover:border-accent/40 transition-colors">
                                        <div className="text-3xl md:text-4xl font-extrabold text-accent mb-1">{metric.value}</div>
                                        <div className="text-xs font-bold text-foreground-secondary uppercase tracking-[0.2em]">{metric.label}</div>
                                    </div>
                                ))}
                            </div>
                        )}
                ...
                <section className="relative px-6 md:px-8 pt-10 pb-4">
                ...
                <section className="relative flex items-center justify-center px-6 md:px-8 pt-16 pb-8 overflow-hidden">
                    <div className="absolute inset-0 pointer-events-none" aria-hidden="true">
                        ...
                        <div className="absolute inset-0 opacity-[0.03] dark:opacity-[0.03] invert dark:invert-0" style={{ backgroundImage: `linear-gradient(rgba(0,0,0,0.1) 1px, transparent 1px), linear-gradient(90deg, rgba(0,0,0,0.1) 1px, transparent 1px)`, backgroundSize: '60px 60px' }} />
                    </div>
            ...
            <SectionWrapper className={`text-center ${densityClass}`.trim()}>
                <div className="max-w-3xl mx-auto py-14 px-8 rounded-3xl bg-accent-gradient shadow-glow relative overflow-hidden">
                    ...
                    <div className="relative z-10">
                        <h2 className="text-3xl md:text-4xl font-bold text-white mb-6">{service.customCta ? service.customCta.title : 'Ready to transform your business?'}</h2>
                        <p className="text-white/90 text-lg mb-6 max-w-xl mx-auto">{inV?.ctaSubtitle ?? (service.customCta ? service.customCta.subtitle : `Let's discuss how we can build your next ${service.title.toLowerCase()}.`)}</p>
                        {service.customCta?.bullets && service.customCta.bullets.length > 0 && (
                        ...
                        <Button onClick={() => openModal('Final')} variant="primary" size="lg" className="!bg-neutral-950 !text-white !bg-none hover:!bg-black border border-white/10 hover:border-white/20 shadow-2xl transition-all font-bold group">
                            {service.customCta ? service.customCta.buttonText : 'Start a project'}
                            <svg width="18" height="18" viewBox="0 0 16 16" fill="none" aria-hidden="true" className="ml-2 group-hover:translate-x-1 transition-transform"><path d="M3 8H13M13 8L9 4M13 8L9 12" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" /></svg>
                        </Button>
                    </div>
                </div>
            </SectionWrapper>
